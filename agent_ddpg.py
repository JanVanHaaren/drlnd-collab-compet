...
TAU = 6e-2  # parameter for soft update of target parameters
UPDATE_NUM = 1  # number of times training
UPDATE_SKIP = 1  # time steps between network updates
...
class Agent:
    ...
    def step(self, state, action, reward, next_state, done, agent_id):
        ...
        if len(self.memory) > BATCH_SIZE:
            if self.time_step % UPDATE_SKIP == 0:
